...
class InstructorRepDB:
    ...
    def add(self, item: Instructor) -> Instructor:
        ...
        id_row = self.db.fetchone("SELECT 1 FROM instructors WHERE instructor_id=%s", (item.instructor_id,))
        if id_row:
        ...
        return Instructor(item.instructor_id, item.last_name, item.first_name, item.patronymic, item.phone, item.experience_years)
